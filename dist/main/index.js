...
只输出 JSON，不要其他内容。`,
  naturalToCommand: (input, shell2) => `
将以下自然语言描述转换为 ${shell2} 命令。
只输出命令本身，不要解释。
...
class MiniMaxProvider {
  ...
  async *naturalToCommand(input, shell2) {
    const prompt = PROMPTS.naturalToCommand(input, shell2);
    yield* this.streamRequest(prompt);
  }
...
function schemaRefOrVal$1({ topSchemaRef, schemaPath }, schema, keyword2, $data) {
  if (!$data) {
    ...
    if (typeof schema == "string")
      return (0, codegen_1$W._)`${schema}`;
  }
  return (0, codegen_1$W._)`${topSchemaRef}${schemaPath}${(0, codegen_1$W.getProperty)(keyword2)}`;
}
...
function makeMergeEvaluated$1({ mergeNames, mergeToName, mergeValues, resultToName }) {
  return (gen, from, to, toName) => {
    const res = to === void 0 ? from : to instanceof codegen_1$W.Name ? (from instanceof codegen_1$W.Name ? mergeNames(gen, from, to) : mergeToName(gen, from, to), to) : from instanceof codegen_1$W.Name ? (mergeToName(gen, to, from), from) : mergeValues(from, to);
    return toName === codegen_1$W.Name && !(res instanceof codegen_1$W.Name) ? resultToName(gen, res) : res;
  };
}
util$1.mergeEvaluated = {
  props: makeMergeEvaluated$1({
    mergeNames: (gen, from, to) => gen.if((0, codegen_1$W._)`${to} !== true && ${from} !== undefined`, () => {
      gen.if((0, codegen_1$W._)`${from} === true`, () => gen.assign(to, true), () => gen.assign(to, (0, codegen_1$W._)`${to} || {}`).code((0, codegen_1$W._)`Object.assign(${to}, ${from})`));
    }),
    mergeToName: (gen, from, to) => gen.if((0, codegen_1$W._)`${to} !== true`, () => {
      if (from === true) {
        gen.assign(to, true);
      } else {
        gen.assign(to, (0, codegen_1$W._)`${to} || {}`);
  ...
  items: makeMergeEvaluated$1({
    mergeNames: (gen, from, to) => gen.if((0, codegen_1$W._)`${to} !== true && ${from} !== undefined`, () => gen.assign(to, (0, codegen_1$W._)`${from} === true ? true : ${to} > ${from} ? ${to} : ${from}`)),
    mergeToName: (gen, from, to) => gen.if((0, codegen_1$W._)`${to} !== true`, () => gen.assign(to, from === true ? true : (0, codegen_1$W._)`${to} > ${from} ? ${to} : ${from}`)),
...
function evaluatedPropsToName$1(gen, ps) {
  ...
  const props = gen.var("props", (0, codegen_1$W._)`{}`);
  if (ps !== void 0)
...
function setEvaluated$1(gen, props, ps) {
  Object.keys(ps).forEach((p) => gen.assign((0, codegen_1$W._)`${props}${(0, codegen_1$W.getProperty)(p)}`, true));
}
...
function getErrorPath$1(dataProp, dataPropType, jsPropertySyntax) {
  if (dataProp instanceof codegen_1$W.Name) {
    const isNumber = dataPropType === Type$1.Num;
    return jsPropertySyntax ? isNumber ? (0, codegen_1$W._)`"[" + ${dataProp} + "]"` : (0, codegen_1$W._)`"['" + ${dataProp} + "']"` : isNumber ? (0, codegen_1$W._)`"/" + ${dataProp}` : (0, codegen_1$W._)`"/" + ${dataProp}.replace(/~/g, "~0").replace(/\\//g, "~1")`;
  }
  return jsPropertySyntax ? (0, codegen_1$W.getProperty)(dataProp).toString() : "/" + escapeJsonPointer$1(dataProp);
}
...
const names$2 = {
  ...
  data: new codegen_1$V.Name("data"),
  // data passed to validation function
  // args passed from referencing schema
  valCxt: new codegen_1$V.Name("valCxt"),
  // validation/data context - should not be used directly, it is destructured to the names below
  instancePath: new codegen_1$V.Name("instancePath"),
  parentData: new codegen_1$V.Name("parentData"),
  parentDataProperty: new codegen_1$V.Name("parentDataProperty"),
  rootData: new codegen_1$V.Name("rootData"),
  // root data - same as the data passed to the first/top validation function
  dynamicAnchors: new codegen_1$V.Name("dynamicAnchors"),
  // used to support recursiveRef and dynamicRef
  // function scoped variables
  vErrors: new codegen_1$V.Name("vErrors"),
  // null or array of validation errors
  errors: new codegen_1$V.Name("errors"),
  // counter of validation errors
  this: new codegen_1$V.Name("this"),
  // "globals"
  self: new codegen_1$V.Name("self"),
  scope: new codegen_1$V.Name("scope"),
  // JTD serialize/parse name for JSON string and position
  json: new codegen_1$V.Name("json"),
  jsonPos: new codegen_1$V.Name("jsonPos"),
  jsonLen: new codegen_1$V.Name("jsonLen"),
  jsonPart: new codegen_1$V.Name("jsonPart")
};
...
function coerceAndCheckDataType$1(it, types2) {
  ...
  const checkTypes = types2.length > 0 && !(coerceTo.length === 0 && types2.length === 1 && (0, applicability_1$2.schemaHasRulesForType)(it, types2[0]));
  if (checkTypes) {
...
function coerceData$1(it, types2, coerceTo) {
  const { gen, data, opts } = it;
  const dataType2 = gen.let("dataType", (0, codegen_1$U._)`typeof ${data}`);
  const coerced = gen.let("coerced", (0, codegen_1$U._)`undefined`);
  if (opts.coerceTypes === "array") {
    gen.if((0, codegen_1$U._)`${dataType2} == 'object' && Array.isArray(${data}) && ${data}.length == 1`, () => gen.assign(data, (0, codegen_1$U._)`${data}[0]`).assign(dataType2, (0, codegen_1$U._)`typeof ${data}`).if(checkDataTypes$1(types2, data, opts.strictNumbers), () => gen.assign(coerced, data)));
  }
  gen.if((0, codegen_1$U._)`${coerced} !== undefined`);
  ...
  function coerceSpecificType(t2) {
    switch (t2) {
      case "string":
        gen.elseIf((0, codegen_1$U._)`${dataType2} == "number" || ${dataType2} == "boolean"`).assign(coerced, (0, codegen_1$U._)`"" + ${data}`).elseIf((0, codegen_1$U._)`${data} === null`).assign(coerced, (0, codegen_1$U._)`""`);
        return;
      case "number":
        gen.elseIf((0, codegen_1$U._)`${dataType2} == "boolean" || ${data} === null
              || (${dataType2} == "string" && ${data} && ${data} == +${data})`).assign(coerced, (0, codegen_1$U._)`+${data}`);
        return;
      case "integer":
        gen.elseIf((0, codegen_1$U._)`${dataType2} === "boolean" || ${data} === null
              || (${dataType2} === "string" && ${data} && ${data} == +${data} && !(${data} % 1))`).assign(coerced, (0, codegen_1$U._)`+${data}`);
        return;
      case "boolean":
        gen.elseIf((0, codegen_1$U._)`${data} === "false" || ${data} === 0 || ${data} === null`).assign(coerced, false).elseIf((0, codegen_1$U._)`${data} === "true" || ${data} === 1`).assign(coerced, true);
        return;
      case "null":
        gen.elseIf((0, codegen_1$U._)`${data} === "" || ${data} === 0 || ${data} === false`);
        gen.assign(coerced, null);
        return;
      case "array":
        gen.elseIf((0, codegen_1$U._)`${dataType2} === "string" || ${dataType2} === "number"
              || ${dataType2} === "boolean" || ${data} === null`).assign(coerced, (0, codegen_1$U._)`[${data}]`);
    }
  }
}
function assignParentData$1({ gen, parentData, parentDataProperty }, expr) {
  gen.if((0, codegen_1$U._)`${parentData} !== undefined`, () => gen.assign((0, codegen_1$U._)`${parentData}[${parentDataProperty}]`, expr));
}
function checkDataType$1(dataType2, data, strictNums, correct = DataType$1.Correct) {
  const EQ = correct === DataType$1.Correct ? codegen_1$U.operators.EQ : codegen_1$U.operators.NEQ;
  let cond;
  switch (dataType2) {
    case "null":
      return (0, codegen_1$U._)`${data} ${EQ} null`;
    case "array":
      cond = (0, codegen_1$U._)`Array.isArray(${data})`;
      break;
    case "object":
      cond = (0, codegen_1$U._)`${data} && typeof ${data} == "object" && !Array.isArray(${data})`;
      break;
    case "integer":
      cond = numCond((0, codegen_1$U._)`!(${data} % 1) && !isNaN(${data})`);
      break;
    case "number":
      cond = numCond();
      break;
    default:
      return (0, codegen_1$U._)`typeof ${data} ${EQ} ${dataType2}`;
  }
  return correct === DataType$1.Correct ? cond : (0, codegen_1$U.not)(cond);
  function numCond(_cond = codegen_1$U.nil) {
    return (0, codegen_1$U.and)((0, codegen_1$U._)`typeof ${data} == "number"`, _cond, strictNums ? (0, codegen_1$U._)`isFinite(${data})` : codegen_1$U.nil);
  }
}
...
function checkDataTypes$1(dataTypes, data, strictNums, correct) {
  ...
  if (types2.array && types2.object) {
    const notObj = (0, codegen_1$U._)`typeof ${data} != "object"`;
    cond = types2.null ? notObj : (0, codegen_1$U._)`!${data} || ${notObj}`;
    delete types2.null;
    delete types2.array;
    delete types2.object;
  } else {
    cond = codegen_1$U.nil;
  }
  if (types2.number)
    delete types2.integer;
  for (const t2 in types2)
    cond = (0, codegen_1$U.and)(cond, checkDataType$1(t2, data, strictNums, correct));
  return cond;
}
dataType$1.checkDataTypes = checkDataTypes$1;
const typeError$1 = {
  message: ({ schema }) => `must be ${schema}`,
  params: ({ schema, schemaValue }) => typeof schema == "string" ? (0, codegen_1$U._)`{type: ${schema}}` : (0, codegen_1$U._)`{type: ${schemaValue}}`
};
...
function checkReportMissingProp$1(cxt, prop) {
  ...
  gen.if(noPropertyInData$1(gen, data, prop, it.opts.ownProperties), () => {
    cxt.setParams({ missingProperty: (0, codegen_1$T._)`${prop}` }, true);
    cxt.error();
  });
}
code$2.checkReportMissingProp = checkReportMissingProp$1;
function checkMissingProp$1({ gen, data, it: { opts } }, properties2, missing) {
  return (0, codegen_1$T.or)(...properties2.map((prop) => (0, codegen_1$T.and)(noPropertyInData$1(gen, data, prop, opts.ownProperties), (0, codegen_1$T._)`${missing} = ${prop}`)));
}
...
function hasPropFunc$1(gen) {
  return gen.scopeValue("func", {
    ...
    code: (0, codegen_1$T._)`Object.prototype.hasOwnProperty`
  });
}
code$2.hasPropFunc = hasPropFunc$1;
function isOwnProperty$1(gen, data, property) {
  return (0, codegen_1$T._)`${hasPropFunc$1(gen)}.call(${data}, ${property})`;
}
code$2.isOwnProperty = isOwnProperty$1;
function propertyInData$1(gen, data, property, ownProperties) {
  const cond = (0, codegen_1$T._)`${data}${(0, codegen_1$T.getProperty)(property)} !== undefined`;
  return ownProperties ? (0, codegen_1$T._)`${cond} && ${isOwnProperty$1(gen, data, property)}` : cond;
}
code$2.propertyInData = propertyInData$1;
function noPropertyInData$1(gen, data, property, ownProperties) {
  const cond = (0, codegen_1$T._)`${data}${(0, codegen_1$T.getProperty)(property)} === undefined`;
  return ownProperties ? (0, codegen_1$T.or)(cond, (0, codegen_1$T.not)(isOwnProperty$1(gen, data, property))) : cond;
}
...
function callValidateCode$1({ schemaCode, data, it: { gen, topSchemaRef, schemaPath, errorPath }, it }, func, context, passSchema) {
  const dataAndSchema = passSchema ? (0, codegen_1$T._)`${schemaCode}, ${data}, ${topSchemaRef}${schemaPath}` : data;
  const valCxt = [
    [names_1$a.default.instancePath, (0, codegen_1$T.strConcat)(names_1$a.default.instancePath, errorPath)],
    [names_1$a.default.parentData, it.parentData],
    [names_1$a.default.parentDataProperty, it.parentDataProperty],
    [names_1$a.default.rootData, names_1$a.default.rootData]
  ];
  if (it.opts.dynamicRef)
    valCxt.push([names_1$a.default.dynamicAnchors, names_1$a.default.dynamicAnchors]);
  const args = (0, codegen_1$T._)`${dataAndSchema}, ${gen.object(...valCxt)}`;
  return context !== codegen_1$T.nil ? (0, codegen_1$T._)`${func}.call(${context}, ${args})` : (0, codegen_1$T._)`${func}(${args})`;
}
code$2.callValidateCode = callValidateCode$1;
const newRegExp$1 = (0, codegen_1$T._)`new RegExp`;
function usePattern$1({ gen, it: { opts } }, pattern2) {
  ...
  return gen.scopeValue("pattern", {
    key: rx.toString(),
    ref: rx,
    code: (0, codegen_1$T._)`${regExp.code === "new RegExp" ? newRegExp$1 : (0, util_2$3.useFunc)(gen, regExp)}(${pattern2}, ${u})`
  });
}
...
function validateArray$1(cxt) {
  ...
  function validateItems(notValid) {
    const len = gen.const("len", (0, codegen_1$T._)`${data}.length`);
    gen.forRange("i", 0, len, (i) => {
      ...
      gen.if((0, codegen_1$T.not)(valid2), notValid);
    });
  }
}
...
function validateUnion$1(cxt) {
  ...
  gen.block(() => schema.forEach((_sch, i) => {
    ...
    gen.assign(valid2, (0, codegen_1$T._)`${valid2} || ${schValid}`);
    const merged = cxt.mergeValidEvaluated(schCxt, schValid);
    if (!merged)
      gen.if((0, codegen_1$T.not)(valid2));
  }));
...
function countKeys$1(schema) {
  ...
  for (const key in schema) {
    ...
    if (typeof schema[key] == "object") {
      (0, util_1$O.eachItem)(schema[key], (sch) => count += countKeys$1(sch));
    }
    if (count === Infinity)
      return Infinity;
  }
...
function requireValidate() {
  ...
  function schemaCxtHasRules2({ schema, self: self2 }) {
    if (typeof schema == "boolean")
      return !schema;
    for (const key in schema)
...
async function saveApiKey(provider, key) {
  await keytar__namespace.setPassword(SERVICE, provider, key);
}
async function getApiKey(provider) {
  return keytar__namespace.getPassword(SERVICE, provider);
}
function getConfig() {
  return {
...
function registerIpcHandlers() {
  require$$1$2.ipcMain.handle(IPC.AI_CHECK_AVAILABLE, async () => {
    const provider = await getProvider();
    return provider.isAvailable();
  });
  require$$1$2.ipcMain.handle(IPC.AI_EXPLAIN_ERROR, async (event, ctx) => {
    const provider = await getProvider();
    return provider.explainError(ctx);
  });
  require$$1$2.ipcMain.handle(IPC.AI_NATURAL_CMD, async (event, input, shell2) => {
    const provider = await getProvider();
    return provider.naturalToCommand(input, shell2);
  });
  require$$1$2.ipcMain.handle(IPC.AI_EXPLAIN_CMD, async (event, command) => {
    const provider = await getProvider();
    return provider.explainCommand(command);
  });
  require$$1$2.ipcMain.handle(IPC.CONFIG_GET, () => {
    return getConfig();
  });
  require$$1$2.ipcMain.handle(IPC.CONFIG_SET, (event, key, value) => {
    if (key === "provider") {
      setProvider(value);
  ...
  require$$1$2.ipcMain.handle(IPC.KEY_GET, async (event, provider) => {
    return getApiKey(provider);
  });
  require$$1$2.ipcMain.handle(IPC.KEY_SAVE, async (event, provider, key) => {
...
function registerPtyHandlers() {
  require$$1$2.ipcMain.handle("pty:create", async (event, shell2, cwd) => {
    const id2 = Math.random().toString(36).slice(2);
    const shellProgram = getShellProgram(shell2);
    const ptyProcess = pty__namespace.spawn(shellProgram, [], {
      cwd,
      cols: 80,
      rows: 24
    });
    ptyProcess.onData((data) => {
      event.sender.send("pty:output", id2, data);
    });
    ptyProcess.onExit(({ exitCode }) => {
      event.sender.send("pty:exit", id2, exitCode);
      instances.delete(id2);
    });
    instances.set(id2, { id: id2, pty: ptyProcess, output: "" });
    return id2;
  });
  require$$1$2.ipcMain.on("pty:input", (event, id2, data) => {
    const instance = instances.get(id2);
    if (instance) {
      instance.pty.write(data);
    }
  });
  require$$1$2.ipcMain.on("pty:resize", (event, id2, cols, rows) => {
    const instance = instances.get(id2);
    if (instance) {
      instance.pty.resize(cols, rows);
    }
  });
  require$$1$2.ipcMain.on("pty:kill", (event, id2) => {
    const instance = instances.get(id2);
    if (instance) {
      instance.pty.kill();
      instances.delete(id2);
...
function getShellProgram(shell2) {
  switch (shell2) {
    case "powershell":
...
function createWindow() {
  mainWindow = new require$$1$2.BrowserWindow({
    ...
    webPreferences: {
      ...
      sandbox: false,
      preload: require$$0__namespace.join(__dirname, "../preload.js")
    },
    show: false,
